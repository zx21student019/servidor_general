...
for x in lista:
        tabla = tabla+"<tr><td>"+(x[1])+"</td>"
        if x[4] == 1:
            tabla += "<td>Si</td>"
            tabla += "<td>No puedes borrar un Admin</td>"
        else :
            tabla += '<td>No</td>'
            tabla += '<td><form action="borrar.py" method="get">'
            tabla += '<button type="submit" class="btn btn-primary">Borrar</button>'
            tabla += '<input type="hidden" name="idUsuario" value="'+str(x[0])+'">'
            tabla += '</form></td>'
#print(tabla)
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-6 text-center">
                    ...
                    <a href='pagPrincipal.py'>Ir a pagina principal</a>
